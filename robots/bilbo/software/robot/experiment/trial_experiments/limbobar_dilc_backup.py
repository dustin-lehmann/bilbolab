...
from robot.experiment.trial_experiments.dilc import DILC_Trial_Meta, DILC_Requirements
...
class LimboBar_DILC_Experiment:
    ...
    Identical to ``DILC_Experiment`` except:
      ...
      - The bar is reset after the robot returns to initial conditions (not
        ...
        return drive).
      - The bar is removed from the testbed on experiment end / error / stop.
    """
    ...
    def run(self) -> LimboBar_DILC_Results | None:
        ...
        if self.settings.target_zone is not None:
            self.logger.info(f"  Target zone: {len(self.settings.target_zone.points)} points")
        else:
            self.logger.info(f"  Target zone: not configured")
        ...
        passes = sum(1 for t in self.trials if t.limbo_bar_passed is True)
        self.logger.info("=" * 60)
        ...
        if self.settings.target_zone is not None:
            self.logger.info(f"  Limbo bar passes: {passes}/{len(self.trials)}")
        if self.trials:
    ...
    def run_trial(self) -> TrialResult:
        try:
            ...
            # --- Step 4b: Read limbo bar hit and passed state ---
