...
def plot_trajectory_grid(outputs):
    ...
    for i, state in enumerate(states):
        ...
        axs[i, 0].plot(outputs['u_balancing'], label='u_external', color='red', linestyle='--')
    ...
    for i, state in enumerate(states):
        ...
        axs[i, 1].plot(outputs['u_input'], label='u_input', color='red', linestyle='--')
        axs[i, 1].plot(outputs['u_output'], label='u_output', color='green', linestyle=':')
...
def save_trajectory_outputs(outputs, experiment_name):
    ...
    filename = f'{timestamp_str}_trajectory_data_{experiment_name}.pkl'
...
def elrond_experiment_test_trajectory(elrond:BILBO):
    ...
    logger.info(f"Running experiment: {name} , N={N}, time={trajectory_time}")
    outputs = {}
    data = {}
    ...
    for i in range(0, N):
        ...
        #     sendValue('theta', outputs[0][i])
        #     sendValue('u_balancing_1', -outputs[1][i])
        #     time.sleep(0.08)

        # Plot the outputs
        #plot_trajectory_outputs(outputs)loading or plotting data: {str(e)}")
        # save the data
...
def stopManualExperimentButton(elrond: BILBO):
    ...
    if start_tick_elrond_experiments is not None and end_tick_elrond_experiments is None:
        ...
        if end_tick_elrond_experiments > start_tick_elrond_experiments:
            outputs = {}
            data = {}
            data = elrond.logging.getData(
                index_start=start_tick_elrond_experiments,
                index_end=end_tick_elrond_experiments,
                signals=signals)

            outputs['v'] = np.asarray(data['lowlevel.estimation.state.v'])
            outputs['theta'] = np.asarray(data['lowlevel.estimation.state.theta'])
            outputs['theta_dot'] = np.asarray(data['lowlevel.estimation.state.theta_dot'])
            outputs['psi_dot'] = np.asarray(data['lowlevel.estimation.state.psi_dot'])

            outputs['u_balancing'] = (np.asarray(data['lowlevel.control.external_input.u_balancing_1'])
                                      + np.asarray(data['lowlevel.control.external_input.u_balancing_2'])) / 2
            outputs['u_input'] = (np.asarray(data['lowlevel.control.data.input_left'])
                                  + np.asarray(data['lowlevel.control.data.input_right'])) / 2

            outputs['u_output'] = (np.asarray(data['lowlevel.control.data.output_left']
                                    + np.asarray(data['lowlevel.control.data.output_right'])) / 2)

            # Plot the outputs
            #plot_trajectory_outputs(outputs)
            # save the data
            save_trajectory_outputs(outputs, f"Manual_experiment")
