...
class IITL_ExperimentPage:
    """Manages a FolderPage for IITL experiments.
    ...
    def __init__(self, robot):
        self.robot = robot
        self.logger = Logger("IITL Page")
        self.experiment: IITL_Experiment | None = None
        self._trial_rows = []
        self._event_handles = []

        self.page = FolderPage(page_id='iitl', name='IITL', rows=10, columns=8)
        self._build_page()
    ...
    def _build_page(self):
        self.status_widget = StatusWidget(
            ...
            elements={
                ...
                'terr': StatusWidgetElement(label='t-err:', color=_COLOR_IDLE, status='--'),
            },
            font_size=9,
        )
        self.page.addObject(self.status_widget, row=1, column=1, width=4, height=2)

        self.trial_table = Table(widget_id='iitl_page_table')
        ...
        self.group_control = Widget_Group(group_id='iitl_page_control', title='Control',
                                          show_title=False, rows=1, columns=8)
        self.page.addObject(self.group_control, row=3, column=1, width=8, height=1)

        self.resume_button = Button(widget_id='iitl_page_resume', text='Resume', color=[0.0, 0.4, 0.0])
        ...
        self.revert_button = Button(widget_id='iitl_page_revert', text='Revert', color=[110 / 255, 82 / 255, 0])
        ...
        self.stop_button = Button(widget_id='iitl_page_stop', text='Stop', color=[0.5, 0.0, 0.0])
        ...
        self.auto_start_button = MultiStateButton(
            id='iitl_page_auto_start', states=['OFF', 'ON'], current_state='OFF',
        ...
        self.auto_accept_button = MultiStateButton(
            id='iitl_page_auto_accept', states=['OFF', 'ON'], current_state='OFF',
        ...
        self.plot_outputs = UpdatableImageWidget(widget_id='iitl_page_plot_outputs')
        self.page.addObject(self.plot_outputs, row=4, column=1, width=4, height=3)

        self.plot_inputs = UpdatableImageWidget(widget_id='iitl_page_plot_inputs')
        self.page.addObject(self.plot_inputs, row=4, column=5, width=4, height=3)

        self.plot_convergence = UpdatableImageWidget(widget_id='iitl_page_plot_convergence')
        self.page.addObject(self.plot_convergence, row=7, column=1, width=4, height=3)

        self.plot_transfer_vector = UpdatableImageWidget(widget_id='iitl_page_plot_transfer_vector')
        self.page.addObject(self.plot_transfer_vector, row=7, column=5, width=4, height=3)

    # === EXPERIMENT BINDING ================================================================================

    def bind_experiment(self, experiment: IITL_Experiment):
        self.unbind_experiment()
        self.experiment = experiment
        self._trial_rows = []

        exp_id = experiment.settings.id if experiment.settings else 'IITL'
        self.status_widget.title = f'IITL -- {exp_id}'
        ...
        self.status_widget.elements['terr'].status = '--'
        ...
        if self.experiment:
            ...
            for i, trial in enumerate(trials):
                ...
                ax.plot(t, np.rad2deg(np.asarray(trial.y_target)),
                        ...
                        label=f"Target {i + 1}" if is_latest else None,
        ...
        if self.experiment:
            ...
            for i, trial in enumerate(trials):
                ...
                ax.plot(t, np.asarray(trial.u_target),
                        ...
                        color=colors[i % max(1, len(colors))],
                        alpha=1.0 if is_latest else 0.25,
                        label=f"u_target {i + 1}" if is_latest else None,
                        zorder=3 if is_latest else 2)
    ...
    def _plot_convergence(self):
        ...
        if self.experiment and self.experiment.settings:
            ...
            if trials:
                ...
                ax.plot(x, [t.learning_error_norm for t in trials],
                        color="C0", lw=2, marker="o", ms=3, label="learning err")
                ...
                if np.any(np.isfinite(e_t)):
                    ax.plot(x, e_t, color="C3", lw=2, marker="s", ms=3, label="transfer err")
                e_m = np.array([t.model_output_error_norm if t.model_output_error_norm is not None
                                else np.nan for t in trials], dtype=float)
                if np.any(np.isfinite(e_m)):
                    ax.plot(x, e_m, color="C2", lw=1.5, marker="^", ms=3, alpha=0.7, label="model err")
                ax.legend(loc="upper right", fontsize=7)
        self.plot_convergence.setFromMatplotLib(fig, dpi=150)

    def _plot_transfer_vector(self):
        fig, ax = self._new_figure()
        ax.set_title("Transfer vector", color="black", fontsize=10)
        ...
        if self.experiment:
            ref = self.experiment.settings.reference_transfer_vector if self.experiment.settings else None
            if ref is not None:
                ax.plot(np.asarray(ref), color="black", lw=1.5, linestyle="--", label="t_ref")
            trials = self.experiment.trials
            if trials and trials[-1].transfer_vector_update is not None:
                ax.plot(np.asarray(trials[-1].transfer_vector_update),
                        color="C0", lw=2, label=f"t (trial {len(trials)})")
            ax.legend(loc="upper right", fontsize=7)
        self.plot_transfer_vector.setFromMatplotLib(fig, dpi=150)
