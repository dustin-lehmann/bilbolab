"""
Experiment Designer Example
===========================

Demonstrates the ExperimentDesignerWidget:
  - YAML-based experiment definition with variables, actions, and loops
  - Play/stop controls wired to ExperimentRunner
  - Playback highlighting shows the currently executing action

A sample experiment is auto-loaded after the GUI starts.
Press the Play button to watch the actions execute step-by-step.

Run from the `software/` directory:
    python -m extensions.gui.examples.advanced.experiment_designer_example
"""
...
def on_play(widget, yaml=''):
    ...
    with _runner_lock:
        if _runner is not None:
            return
    print(f'[experiment] Received YAML ({len(yaml)} chars)')
    ...
    try:
        ...
        print(f'[experiment] Failed to parse: {e}')
    ...
    def step_loop():
        ...
        while True:
            ...
            if finished:
                break
            time.sleep(0.05)
        print(f'[experiment] Finished: {runner.status.value}')
        with _runner_lock:
            _runner = None
...
def main():
    ...
    # Load sample experiment after GUI initialises
    time.sleep(10)
    designer.load_experiment(SAMPLE_EXPERIMENT)
    print('[experiment] Sample experiment loaded')
