...
def on_play(widget, yaml=''):
    """Run loop experiment using the widget's built-in runner connection."""
    global _runner

    with _runner_lock:
        if _runner is not None:
            return
    print(f"[PLAY] Received YAML ({len(yaml)} chars)")
    ...
    def step_loop():
        ...
        # Widget automatically cleans up via the runner's finished event
...
def example_experiment_designer_loop():
    host = getHostIP()
    ...
    print("[INIT] Loop experiment loaded into designer")
...
if __name__ == '__main__':
    example_experiment_designer_loop()
