...
# Set to False to bypass the write buffer and write directly to HDF5.
# When True, append_sample/append_multiple_samples only touch a fast numpy
# buffer (under _buffer_lock). A background thread periodically flushes
# the buffer to h5py (under self.lock). This ensures the update loop is
# never blocked by slow h5py operations from concurrent readers/flushers.
H5_USE_WRITE_BUFFER = True
...
class H5PyDictLogger:
    ...
    def __init__(self, filename, dataset_name="samples", chunk_size=10000,
        ...
        self.file = None
        self.dataset = None
        self.lock = threading.Lock()  # Protects h5py file operations (read/write)
        ...
        # Write buffer: pre-allocated numpy array, filled in-place to avoid GC pressure.
        # Uses a SEPARATE lock (_buffer_lock) so the update loop never blocks on h5py.
        self._buffer_lock = threading.Lock()  # Protects buffer array only (fast)
        self._write_buffer = None  # Allocated in start() once dtype is known
        self._write_buffer_size = 1000  # Large enough to never overflow between flushes
    ...
    def start(self, mode='w'):
        ...
        if self.dataset_name in self.file:
            ...
            self._allocated_size = self.dataset.shape[0]
        else:
        ...
        # Pre-allocate write buffer and start auto-flush thread
        if H5_USE_WRITE_BUFFER and self.dtype is not None:
            self._write_buffer = np.zeros(self._write_buffer_size, dtype=self.dtype)
            self._write_buffer_count = 0
            self._flush_stop = threading.Event()
            self._flush_thread = threading.Thread(
                target=self._auto_flush_loop, daemon=True,
                name=f'h5-flush-{self.dataset_name}',
            )
    ...
    def append_sample(self, sample):
        """
        Appends a single sample. When buffered, only touches the numpy buffer
        under _buffer_lock (fast). A background thread flushes to h5py separately.
        """
        if self.dtype is None:
            return

        # Convert outside any lock (CPU-heavy dict flattening + record conversion)
        if isinstance(sample, dict):
            if self._dict_flatten_cache is not None:
                sample = optimized_flatten_dict(sample, self._dict_flatten_cache)
            sample = self._dict_to_record(sample)

        if H5_USE_WRITE_BUFFER:
            # Only _buffer_lock — never blocks on h5py
            with self._buffer_lock:
                if self._write_buffer_count >= self._write_buffer_size:
                    return  # Buffer overflow — should not happen with proper flush interval
                self._write_buffer[self._write_buffer_count] = sample
                self._write_buffer_count += 1
        else:
            with self.lock:
                new_size = self.current_size + 1
                if new_size > self._allocated_size:
                    self._allocated_size = new_size + self.chunk_size - (new_size % self.chunk_size)
                    self.dataset.resize((self._allocated_size,))
    ...
    def append_multiple_samples(self, samples: list):
        """
        Appends a list of samples. When buffered, only touches the numpy buffer
        under _buffer_lock (fast).
        """
        if self.dtype is None:
            return

        # Convert all records outside any lock
        records = []
        for sample in samples:
            if isinstance(sample, dict):
                if self._dict_flatten_cache is not None:
                    sample = optimized_flatten_dict(sample, self._dict_flatten_cache)
                records.append(self._dict_to_record(sample))
            else:
        ...
        if H5_USE_WRITE_BUFFER:
            ...
            with self._buffer_lock:
                for record in records:
                    ...
                    self._write_buffer[self._write_buffer_count] = record
                    self._write_buffer_count += 1
        else:
    ...
    def flush_write_buffer(self):
        """Flush buffered samples to h5py. Safe to call from any thread.

        Lock ordering: _buffer_lock first (to grab data), then self.lock (for h5py).
        The update loop only ever holds _buffer_lock, so it is never blocked by this.
        """
        if not H5_USE_WRITE_BUFFER:
            return

        # Step 1: Grab data from buffer (fast, brief lock)
        with self._buffer_lock:
            n = self._write_buffer_count
            if n == 0:
                return
            data = self._write_buffer[:n].copy()
            self._write_buffer_count = 0

        # Step 2: Write to h5py (slow, but _buffer_lock is released so update loop continues)
        with self.lock:
            self._write_data_to_h5(data, n)

    def _write_data_to_h5(self, data, n):
        """Write a numpy array of n records to the dataset. Must hold self.lock."""
        ...
        if new_size > self._allocated_size:
            self._allocated_size = new_size + self.chunk_size - (new_size % self.chunk_size)
            self.dataset.resize((self._allocated_size,))
        self.dataset[old_size:new_size] = data[:n]
        self.current_size = new_size
        if (old_size // self.flush_interval) != (new_size // self.flush_interval):
            self.file.flush()

    def _auto_flush_loop(self):
        """Background thread: flush buffer to h5py every 2 seconds."""
        while not self._flush_stop.wait(timeout=2.0):
            try:
    ...
    def get_samples(self, index, signals=None, to_dict: bool = False):
        ...
        # Flush any buffered records so reads see all data
        if H5_USE_WRITE_BUFFER:
            self.flush_write_buffer()
        ...
        def _indices_from_slice(s: slice):
            ...
            stop = s.stop if s.stop is not None else self.current_size
    ...
    def close(self):
        """
        Closes the HDF5 file. Stops flush thread, flushes remaining buffer, trims dataset.
        """
        # Stop auto-flush thread first
        ...
        with self.lock:
            if self.file:
                # Trim dataset to actual size (was over-allocated for performance)
                if self.dataset is not None and self.current_size < self._allocated_size:
                    self.dataset.resize((self.current_size,))
                self.file.flush()
                self.file.close()
                self.file = None
                self.dataset = None
