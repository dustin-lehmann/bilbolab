...
HAL_StatusTypeDef CAN::init(can_config_t config) {
    ...
    for (int i = 0; i < CAN_NUMBER_RR; i++) {
        readRequests[i].motor_id = 0;
        readRequests[i].register_id = 0;
...
void CAN::removeReadRequest(uint32_t motor_id, uint16_t register_id) {
    xSemaphoreTake(this->mapMutex, portMAX_DELAY);
    for (int i = 0; i < CAN_NUMBER_RR; i++) {
        if (readRequests[i].isOpen && readRequests[i].motor_id == motor_id && readRequests[i].register_id == register_id) {
            readRequests[i].isOpen = false;
            readRequests[i].taskHandle = nullptr;
            break;
        }
...
void CAN::onMessageReceived(const FDCAN_RxHeaderTypeDef &header, uint8_t *data) {
    ...
    for (int i = 0; i < CAN_NUMBER_RR; i++) {
        if (readRequests[i].isOpen && readRequests[i].motor_id == header.Identifier && readRequests[i].register_id == register_id) {

        	  memcpy(readRequests[i].responseData, &data[4], readRequests[i].register_data_length);
...
CAN_Status CAN::readMessage(uint32_t motor_id, uint16_t register_id, uint8_t register_data_length, uint8_t* rx_data, uint32_t timeoutMs){

	TaskHandle_t currentTask = xTaskGetCurrentTaskHandle();

	// Build Readrequest
	this->addReadRequest(motor_id, register_id, register_data_length, currentTask);

	// Build Header
    FDCAN_TxHeaderTypeDef TxHeader = {
        .Identifier = motor_id,
        .IdType = FDCAN_STANDARD_ID, // Adjust if standard IDs should also be allowed.
        .TxFrameType = FDCAN_DATA_FRAME,
        .DataLength = mapDLC(register_data_length+4),
        .ErrorStateIndicator = FDCAN_ESI_ACTIVE,
        .BitRateSwitch = FDCAN_BRS_OFF,
        .FDFormat = FDCAN_CLASSIC_CAN,
        .TxEventFifoControl = FDCAN_NO_TX_EVENTS,
    ...
    uint16_to_bytearray(register_id, register_address_array);


    tx_data[0] = 0x41;
    tx_data[1] = 0;
    tx_data[2] = register_address_array[0];
    tx_data[3] = register_address_array[1];


//    HAL_StatusTypeDef status = HAL_FDCAN_AddMessageToTxFifoQ(this->config.hfdcan, &TxHeader, tx_data);
    HAL_FDCAN_AddMessageToTxFifoQ(this->config.hfdcan, &TxHeader, tx_data);
    ...
    if (ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(timeoutMs)) > 0) {
            ...
            for (int i = 0; i < CAN_NUMBER_RR; i++) {
                if (readRequests[i].motor_id == motor_id && readRequests[i].isOpen && readRequests[i].register_id == register_id) {
                    memcpy(rx_data, readRequests[i].responseData, readRequests[i].register_data_length);
                    break;
                }
            }
            xSemaphoreGive(this->mapMutex);
            removeReadRequest(motor_id, register_id);
            return CAN_SUCCESS;
        } else {
            removeReadRequest(motor_id, register_id);
...
bool CAN::addReadRequest(uint32_t motor_id, uint16_t register_id, uint8_t register_data_length, TaskHandle_t taskHandle) {
    xSemaphoreTake(this->mapMutex, portMAX_DELAY);
    for (int i = 0; i < CAN_NUMBER_RR; i++) {
        if (!readRequests[i].isOpen) {

            readRequests[i].motor_id = motor_id;
            readRequests[i].taskHandle = taskHandle;

            readRequests[i].register_data_length = register_data_length;
            readRequests[i].register_id = register_id;

            readRequests[i].responseData[0] = 0;
            readRequests[i].responseData[1] = 0;
            readRequests[i].responseData[2] = 0;
            readRequests[i].responseData[3] = 0;
